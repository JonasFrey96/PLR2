...
class KeypointNet(nn.Module):
    ...
    def forward(self, img, points, label=None):
        ...
        if label is None:
            label = segmentation.argmax(dim=1).to(torch.int64)

        label_small = F.interpolate(label.to(torch.float32)[:, None], size=[240, 320], mode='nearest').round().to(torch.int64)

        keypoints = self.keypoint_head(x, label_small)
