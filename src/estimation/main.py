...
def calculate_print_error(pred, gt):
    print('translation error:', np.sum(np.abs(pred.state[0:3] - gt.state[0:3])))
    print('rotation error squared:', np.sum(np.abs(pred.state[3:] - gt.state[3:7])))
...
if __name__=='__main__':
    ...
    state1_motion_np[0:3, 0:3] = R.from_euler('zyx', np.array([20, 10, 13]), degrees=True).as_matrix()
    ...
    print('state3 =\n', state3)
    print('state2_3 == state1_2, with error', np.sum(abs(state2_3.state - state1_2.state)))
    ...
    print('r3xq1 =\n', r3xq1)
    print('r3xq1_motion =\n', r3xq1_motion)
    print('State_SE3 from r3xq1_motion == state1_motion, with error', np.sum(np.abs(State_SE3(r3xq1_motion).state - state1_motion.state)))
    ...
    print('r3xq1_2 == r3xq2_3, with error', np.sum(np.abs(r3xq2_3.state - r3xq1_2.state)))
    ...
    print('lm0_1 == r3xq1_2, with error', np.sum(np.abs(r3xq1_2.state - lm0_1.state)))
    lm1_2 = LM.interpolate_motion(trajectory, 2)
    print('lm1_2 == r3xq2_3, with error', np.sum(np.abs(r3xq2_3.state - lm1_2.state)))

    print('Check extrapolate_motion.')
    lm2_exp = LM.extrapolate_motion(trajectory, 1, lm1_2)
    print('lm2_exp == trajectory[2], with error', np.sum(np.abs(lm2_exp.state - trajectory[2].state)))
    ...
    print('Points1 transformed with identity transform is the same with error', np.sum(np.abs(points1.coords - points1_t.coords)))

    print('Testing ADD error.')
    print('ADD error for same two poses done on points is', ADD(points1, r3xq1_motion, r3xq1_2))

    points1_s = points([[-1, 0, 0], [1, 0, 0], [0, -1, 0], [0, 1, 0]]) # symmetric 4 points along z axis
    pose1 = State_R3xQuat() 
    pose1.state_from_R3xYPR([0, 0, 0, pi/2, 0, 0])# rotation along z axis of 90 degrees
    pose2 = State_R3xQuat()
    pose2.state_from_R3xYPR([0, 0, 0, -pi/2, 0, 0]) # rotation along z axis of -90 degrees
    
    print('ADD-S error for symmetric points with two poses along axis of symmetry is ', ADDS(points1_s, pose1, pose2))
    ...
    print('diag(KF2.covariances[0]) as prior variances_motion_1 (high),', np.diag(KF2.covariances[0]))
