...
class ConfigLoader(object):
    ...
    # -> ConfigLoader:
    def from_file(self, env_path: str = 'yaml/env.yml', suppress_print=False):
    ...
    def from_string(self, data):  # -> ConfigLoader: # data: str
        self.env = yaml.load(data, Loader=FullLoader)
        return self

    def switch(self, key, options):  # key: str, options: Dict[str, Callable]
        value = self.__getitem__(key)
        return options[value]()

    def get_FullLoader(self):
        return self.env
    ...
    def __getitem__(self, key):
        if isinstance(key, list) or isinstance(key, tuple):
            ...
            for k in key:
                ...
                existence_check = len(val) > k if isinstance(
                    val, list) else (k in val)
                if type_check and existence_check:
                    val = val[k]
                else:
                    raise ValueError(
                        "Error : Could not retrieve key {} in  {}".format(k, key))
            return val
        else:
            subkeys = key.split('/')
            if len(subkeys) > 1:
        ...
        if isinstance(key, list) or isinstance(key, tuple):
            ...
            for id_k, k in enumerate(key):
                ...
                if type_check and existence_check and id_k == len(key) - 1:
                    ...
                    raise ValueError(
                        "Error : Could not store key {} in  {}".format(k, key))
        else:
            subkeys = key.split('/')
            if len(subkeys) > 1:
